...
针对市场= 1 Tokey; 2 London; 3 New York 在开市之后一分钟,回数4根M15蜡烛,即开市前一小时和开市后一小时的9根棒
取最高点和最低点,下两个挂单.长单的入场价格为一最高点,止损在最低点.
如果做北美市场,则关注的货币对应与USD相关,例如GBPUSD.
原作者介绍,目标获利大约是30-50点,也可以同时设追迹15点.如果最高最低点的差在50-60以下,获利会大一些.
...
3 设止赢和追迹止损.
**关于时间问题:
**开市后一个小时,纽约当地时间 8:00, 要折算为服务器时间服务器时差 1, 市场时差-5.
...
extern int    iHalfPosHLDist       = 160;  //**高点到低点的差,高于此点,头寸减半,如果头寸为帐户最低开仓量则头寸为0. 140-180? =180
extern int    pipplus              = 2;  //**开仓点位加的点数
extern int    slippage             = 3;
extern bool   UseFirstTarget       = false;  //获利到达此点时,平一半仓位
extern int    iFirstTarget         = 90; //** 70-80? =80
extern double Lots                 = 0.1;  //如果以下值为真,则不用此值.
...
double dP, dSL, dTP;  //下挂单的价格,止损和止赢
double Lots1, Ilo;
string filename;
int cnt;
int iStatus = 0;
...
void LotOptimized()
   ...
   if (iH-iL>iHalfPosHLDist*Point) Lots1=Lots1/2;  //如果最高低点差太大,则头寸减半
   if (Lots1<MarketInfo(Symbol(),MODE_MINLOT)) Lots1 = 0;  //如果头寸小于帐户最低头寸,头寸为0,意味着不开仓
   }
