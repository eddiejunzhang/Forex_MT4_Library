...
//--- 
   ...
   Print("zMA=",zMA,", iBOLL=",iBOLL,", zM15KDJ=",zM15KDJ,", zH1KDJ=",zH1KDJ);    // for debug.
...
//为了防止持续发送通知，设一个参数bSendNotice.仅当zTotall值发生变化时，才会发送通知。
//   if(zTotall!=0 && bSendNotice)
//     {
//      string H4KDJTrend;
...
//                       "[Index]="+IntegerToString(zTotall,2));
//      bSendNotice=false;
//     }
      ...
      if(CheckOrders())
        {
         ...
         if(AllowPlaceLongOrder && OrdersTotal()<1)
           ...
           {
            //通知，出现长单交易机会，但不允许下单。
            SendNoticeToPhone("长单机会I "+
                              "[zMA]="+DoubleToString(zMA,0)+
                              "[iBOLL+zM15KDJ+zH1KDJ]="+IntegerToString(iBOLL+zM15KDJ+zH1KDJ,0));
           }
        }
     }
   else
